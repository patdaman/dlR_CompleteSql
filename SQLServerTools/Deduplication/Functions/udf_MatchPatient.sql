...
BEGIN
	...
	FROM Patient P
		INNER JOIN PatientAddress PA ON P.id = PA.PatientId
			AND (dbo.udf_CLR_LevenshteinDistance(PA.Address1, @Address1, 1) <= 3) 
	WHERE CONVERT(date, P.DateOfBirth) = @DateOfBirth
		AND 
		(	(dbo.udf_CLR_LevenshteinDistance(P.FirstName, @FirstName, 1) <= 3
				AND dbo.udf_CLR_LevenshteinDistance(P.LastName, @LastName, 1) <= 3
			) 
